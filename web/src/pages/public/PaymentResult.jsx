...
const OK_ORDER_STATUSES = ["PAID", "CONFIRMED", "PREPARING", "SHIPPING", "COMPLETED", "DONE"];
...
export default function PaymentResultPage() {
  ...
  const [state, setState] = useState({
    loading: true, msg: "Đang kiểm tra thanh toán…", orderId: null,
    status: null, hintOk: false, error: null,
  });
  ...
  useEffect(() => {
    ...
    setState(prev => ({
      ...
      orderId, status: queryLooksOk ? "PAID" : null, hintOk: queryLooksOk, error: null,
    }));
    ...
    async function poll() {
      ...
      try {
        ...
        if (OK_ORDER_STATUSES.includes(currentBEStatus)) {
          ...
          try { await clearCart(); setCount(0); } catch (cartError) { console.warn("Lỗi khi xóa giỏ hàng:", cartError); }
        ...
        if (tries >= 5) {
          setState({
            loading: false, msg: "Không thể kiểm tra trạng thái đơn hàng lúc này. Vui lòng kiểm tra lại sau.",
            orderId, status: null, hintOk: queryLooksOk, error: e?.message || "Lỗi không xác định",
          });
          return;
        }
  ...
  return (
    <div className="container section fade-in">
       <div className={`card card-hover ${finalOk ? 'border-green-300' : 'border-red-300'}`}>
