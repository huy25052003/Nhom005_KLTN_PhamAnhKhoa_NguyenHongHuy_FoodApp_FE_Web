...
export default function AnalyticsPage() {
  ...
  async function load() {
    setLoading(true);
    setOverview({ revenue:0, orders:0, items:0, avgOrderValue:0 });
    setSeries([]); setTops([]); setStatus([]); setLow([]);
    try {
      const [ov, se, tp, st, lw] = await Promise.all([
        getOverview({ from, to }).catch(() => ({ revenue:0, orders:0, items:0, avgOrderValue:0 })),
        getRevenueSeries({ from, to }).catch(() => []),
        getTopProducts({ from, to, limit: 10 }).catch(() => []),
        getOrdersByStatus({ from, to }).catch(() => []),
        getLowStock({ threshold: 10, limit: 10 }).catch(() => []),
      ]);
      setOverview(ov);
      setSeries(se);
      setTops(tp);
  ...
  return (
    <div className="page-analytics">
      ...
      <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap", marginBottom:12, padding: '12px', background: '#fff', borderRadius: '12px', border: '1px solid var(--border)' }}>
        <label>Từ</label>
        ...
        <div style={{ display:"flex", gap:8, marginLeft: 'auto' }}>
          <button className="btn btn-ghost" onClick={()=> { setFrom(addDays(todayStr(), -6)); setTo(todayStr()); }}>7 ngày</button>
          <button className="btn btn-ghost" onClick={()=> { setFrom(addDays(todayStr(), -29)); setTo(todayStr()); }}>30 ngày</button>
          <button className="btn btn-ghost" onClick={()=> { setFrom(addDays(todayStr(), -89)); setTo(todayStr()); }}>90 ngày</button>
      ...
      <div className="grid4">
        {loading ? (
            Array.from({length: 4}).map((_, i) => <div key={i} className="card stat placeholder"><div className="muted">Đang tải...</div></div>)
        ) : (
          <>
      ...
      <div className="grid2" style={{ marginTop:12 }}>
        <div className="card" style={{ overflow:"hidden" }}>
          <div className="card-title">Top sản phẩm bán chạy</div>
           {loading ? <div className="muted" style={{padding: 16}}>Đang tải...</div> : (
                ...
                <tbody>
                  {tops.length ? tops.map((t,i)=>(
                    <tr key={t.productId}>
                     ...
                     <tr><td colSpan={4} className="muted" style={{ textAlign:"center", padding:12 }}>Không có dữ liệu</td></tr>
                  )}
                </tbody>
              </table>
            )}
                  ...
                  return (
                    <div key={s.status}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom: '4px' }}>
                        ...
                        <div className="muted">{s.count} ({pct}%)</div>
                      </div>
      ...
      <div className="card" style={{ overflow:"hidden", marginTop:12 }}>
        <div className="card-title">Cảnh báo tồn kho thấp (&lt;= 10)</div>
         {loading ? <div className="muted" style={{padding: 16}}>Đang tải...</div> : (
            <table className="table">
              <thead><tr><th>ID</th><th>Tên</th><th>Tồn kho</th></tr></thead>
              <tbody>
                {low.length ? low.map(r => (
                  <tr key={r.id}><td>{r.id}</td><td>{r.name}</td><td>{r.stock}</td></tr>
                )) : (
                  <tr><td colSpan={3} className="muted" style={{ textAlign:"center", padding:12 }}>Không có sản phẩm nào sắp hết hàng</td></tr>
                )}
              </tbody>
            </table>
          )}
       ...
       <style>{`
        .stat.placeholder { min-height: 100px; display: flex; align-items: center; justify-content: center; }
